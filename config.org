...
** Folders are suggestions, not requirements

Every folder under your org directory is just a folder: it lives in the
agenda, and capture asks you which one to file into. Create whatever
structure suits you. The table below shows a /suggested/ convention  
nothing is auto-created:

| Folder       | Suggested use                                                                   |
|--------------+---------------------------------------------------------------------------------|
| =inbox.org=  | unsorted front door (one file you can pick during capture)                      |
...
| =notes/=     | =:concept:= Zettel and =:collection:= items (movies, places   with TODO states) |
| =*/archive/= | archive/ folders are the only ones excluded from the agenda                     |

The agenda scans your org directory recursively (minus =archive/= folders),
rebuilt before each =M-m a=. Add a folder and it shows up; move something
...
for a Laborprotokoll, context ("Lieferant angerufen, M?ller meldet
...
Everything you will ever do in this system is one of these moves:

1. *Capture*   =M-m c= (=m= meeting,
   =l= Laborprotokoll). Every template prompts for the
...
3. *Change note*   =M-m M-z=. The manual twin of the state change: any
   ...
   PEEK   NBR-Dichtungen versagen ?ber 140 ?C." The note is the
...
finds the node via search   "Lieferantenproblem" helps nobody; "M?ller
...
*** Debounced state-change logging
When cycling through states rapidly, only log the final state after 1.5s idle.
#+begin_src emacs-lisp
