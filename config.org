...
** Folders encode behavior, tags encode type

A folder exists only where files /behave/ differently — agenda
membership, archiving ritual, mutability. Where only the /kind/ of
content differs, a =#+filetags= entry distinguishes it. That is why
there is no =labor/= folder (a protocol and a field report behave
identically → both are =records/= with different tags), and why
concept Zettel and collection items share =notes/=.

| Folder     | Shape          | Behavior                                                                                                            |
|------------+----------------+---------------------------------------------------------------------------------------------------------------------|
| =inbox.org=  | not yet sorted | single front door; emptied at the daily review by refiling                                                          |
| =dailies/=   | moment-record  | journal, one file per day; optional; never edited after its day                                                     |
| =records/=   | moment-record  | ALL typed records (=:labor:=, =:meeting:=, …); immutable; link projects by ID                                           |
| =projects/=  | entity         | one file per project; IN the agenda; project ends → whole file to =projects/archive/=                                 |
| =areas/=     | entity         | machines, process domains, teams; IN the agenda; never end; superseded subtrees → =areas/archive/=                    |
| =notes/=     | understanding  | =:concept:= Zettel (rewrite freely) and =:collection:= items (movies, places — with TODO lifecycles); in the agenda |
| =*/archive/= | memory         | out of the agenda, fully inside search and the org-roam graph                                                       |

A new record type (Feldbericht, Audit, …) is one new capture template
with its own filetag. A new note type is one new roam template. The
folder set above is final.
...
1. *Capture* — =M-m c= (=t= todo, =n= note, =d= decision, =m= meeting,
   =l= Laborprotokoll, =p= project). Everything enters through the
   inbox or a template; ID and =CREATED= stamp are automatic, and
   file captures (Projekt, Laborprotokoll) get a file-level ID, so
   every captured thing is a linkable roam node from birth. You never
   create entities by typing raw headings mid-thought — and that is
...
| Situation                                        | Do this                                                                      |
|--------------------------------------------------+------------------------------------------------------------------------------|
| Anything incoming — idea, request, action needed | =M-m c t= → inbox; shape it at the daily review                              |
...
#+begin_src emacs-lisp
  ...
  (use-package org
    ...
    ;; Agenda = the WORKSPACE. dailies/, records/ and every archive/
    ;; stay out on purpose — pure moment-records and memory don't belong
    ;; in a todo view. notes/ IS included, so :collection: items with
    ;; TODO states (films to watch, places to visit) surface here.
    ;; Follow-ups noticed while writing a protocol are captured as tasks
    ;; (M-m c t), not typed into the record.
    ;; (If you DON'T want collection TODOs in the agenda, drop
    ;; my/org-notes-dir from this list.) `my/org-agenda-subdir' peeks anywhere.
    (org-agenda-files (list my/org-dir my/org-projects-dir
                            my/org-areas-dir my/org-notes-dir))
    :config
...
#+begin_src emacs-lisp
;; Records of any type share records/ and a filename scheme of
;; YYYYMMDD_titel.org; the TYPE is a #+filetags entry. A new record type
;; (Feldbericht, Audit, …) = one more template with its own filetag —
;; never a new folder.

(defun my/org-project-id-link ()
  "Prompt for a project and return an id link to it, or \"\" for none.
Ensures the chosen project file has a file-level ID (creating one if
needed), so org-roam backlinks list every record taken for the project."
  (let* ((files (directory-files my/org-projects-dir nil "\\.org\\'"))
         (choice (completing-read "Projekt (leer = keins): " files nil nil)))
    (if (or (null choice) (string-empty-p choice))
        ""
      (with-current-buffer
          (find-file-noselect (expand-file-name choice my/org-projects-dir))
        (save-excursion
          (goto-char (point-min))
          (let ((id (org-id-get-create)))
            (save-buffer)
            (format "[[id:%s][%s]]" id (file-name-base choice))))))))

(defvar mein/labor-titel nil)
(defun mein/labor-capture-datei ()
  (setq mein/labor-titel (read-string "Versuchsbezeichnung: "))
  (concat my/org-records-dir
  ...
  (setq org-capture-templates
        '(("l" "Laborprotokoll" plain
          ...
          ("n" "Note" entry
           ...
           "* %?\n  %U\n")
          ;; --- One Round System additions ---
          ;; Decisions are ENTITIES: captured ACTIVE into the inbox, then
          ;; refiled to the project's/area's Decisions heading. Overruled
          ;; later? M-m x s (supersede), never edit in place.
          ("d" "Decision" entry
           (file+headline "inbox.org" "Inbox")
           "* ACTIVE %^{Entscheidung}\n  %U\n  Kontext: %a\n  %?")
          ;; Meetings are just another RECORD TYPE: a datetree of minutes
          ;; in records/, written once, never edited later. Decisions made
          ;; in the meeting get their own "d" capture and are linked here.
          ("m" "Meeting" entry
           (file+olp+datetree "records/meetings.org")
           "* %^{Thema}\n  - Teilnehmer :: %^{Teilnehmer}\n  - Projekt :: %(my/org-project-id-link)\n  %?"))))
#+end_src

** One Round System — lifecycle commands
The five rituals and where they live:
  1. Capture       M-m c     everything enters via the inbox (t/n/d/m/l/p)
...
#+begin_src emacs-lisp
;; Every capture becomes a node. ENTRY captures (todo, decision,
;; meeting) get ID + CREATED on the heading; PLAIN file captures
;; (Projekt, Laborprotokoll, future Feldbericht) get a FILE-LEVEL ID —
;; the :PROPERTIES: drawer above #+title — so the new file is linkable
;; as a roam node AND the links it contains (e.g. the record's
;; "Projekt ::" line) register as backlinks on their targets.
;; org-roam's own captures are skipped: roam manages those IDs itself,
;; and dailies would drown in property drawers.
