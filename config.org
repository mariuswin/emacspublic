  ...
  ;; (Buffer switching keeps live preview: buffers are already loaded, cheap.)
   ...
   (setq consult-fd-args
         '((if (executable-find "fdfind" 'remote) "fdfind" "fd")
           "--full-path --color=never --no-ignore"))
...
(with-eval-after-load 'dired
  ...
  (keymap-set dired-mode-map "C-o" #'consult-fd))
#+end_src
...
;; C-k opens roam node completion (pick or create a node as usual,
;; nodes visible immediately). If you paste a URL (http/https/ftp, or
;; starting with www.) into the completion, it is inserted as a website
;; link instead; an active region becomes the description, otherwise
;; you are prompted. New nodes use one no-choice template and open for
;; editing.
...
(defun my/ck--maybe-url-redirect ()
  "In the roam node minibuffer: redirect C-k to a website link when a
URL is pasted/typed."
  (let ((s (minibuffer-contents-no-properties)))
    (when (string-match-p "\\`\\(https?://\\|ftp://\\|www\\.\\)" s)
      (throw 'my/ck--url (cons 'url s)))))

(defun my/org-roam-node-insert-quick ()
  "Insert a link. Roam node completion is the default (pick or create a
node as usual, nodes visible immediately). If you paste a URL
(http/https/ftp, or starting with www.) into the completion it is
inserted as a website link instead; an active region becomes the
description, otherwise you are prompted (default = the URL)."
  (interactive)
  (let ((created nil)
        (grab (lambda ()
          ...
          (let ((result
                 (catch 'my/ck--url
                   ...
                   (minibuffer-with-setup-hook
                       (lambda ()
                         (add-hook 'post-command-hook #'my/ck--maybe-url-redirect nil t))
                     (org-roam-node-insert))
                   'node)))
            (when (eq (car-safe result) 'url)
              (let* ((u    (cdr result))
                     (url  (if (string-match-p "\\`www\\." u)
                               (concat "https://" u) u))
                     (desc (if (region-active-p)
                               (buffer-substring-no-properties (region-beginning) (region-end))
                             (read-string "Description: " url))))
                (if (region-active-p)
                    (progn (delete-region (region-beginning) (region-end))
                           (insert (org-link-make-string url desc)))
                  (insert (org-link-make-string url desc))))))
...
(global-set-key (kbd "C-o")   #'consult-fd)            ;; fuzzy file search via fd (recursive)
...
(use-package magit
  ...
  :config
  (define-key magit-mode-map (kbd "C-w")      #'kill-current-buffer)
  (define-key magit-mode-map (kbd "C-<tab>")  #'consult-buffer))
#+end_src
...
The full file verions can then be shown with "git show ..."
...
(defun my/start-screen ()
  ...
  (with-current-buffer (get-buffer-create my/start-buffer-name)
    (let ((inhibit-read-only t))
      ...
      (my/start--key "C-k"     "Node link; paste a URL -> website link")
